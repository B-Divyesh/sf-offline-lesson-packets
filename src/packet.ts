...
<body><main class="packet"><header class="packet-head"><h1>${title}</h1><div class="meta">${subject ? `<span>${subject}</span>` : ''}${packet.minutes ? `<span>About ${packet.minutes} minutes</span>` : ''}<span>Works offline</span></div><p class="instructions">${instructions}</p><label class="learner" for="learner-name">Your name or initials (optional)<input id="learner-name" autocomplete="name" maxlength="100"></label><p class="privacy">Nothing in this packet is sent anywhere.${packet.allowLocalSave && !preview ? ' Your progress is remembered only in this browser.' : ''}</p></header>
